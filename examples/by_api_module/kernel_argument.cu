...
static_assert(!std::is_trivially_copyable<NonTriviallyCopyable>::value,
              "NonTriviallyCopyable should not be trivially copyable");

template <>
struct cuda::traits::is_valid_kernel_argument<NonTriviallyCopyable>
    : ::std::true_type {};
...
int main(int argc, char **argv) {
  ...
  cuda::device::id_t device_id =
      (argc > 1) ? std::stoi(argv[1]) : cuda::device::default_device_id;
  ...
  std::cout << "Using CUDA device " << device.name() << " (having device ID "
            << device.id() << ")\n";

  {
    auto stream =
        cuda::device::current::get().create_stream(cuda::stream::sync);
    std::cout << "A new CUDA stream with no priority specified defaults to "
                 "having priority "
              << stream.priority() << ".\n";
    stream.enqueue.kernel_launch(print_message, single_thread_config,
                                 NonTriviallyCopyable("It works!"));
