...
void sequence_cpu(int *h_ptr, size_t length)
{
	...
	{
		h_ptr[elemID] = elemID % cuda_::warp_size;
	}
}
...
int main(int, char **)
{
	...
	cuda_::device::current::set_to_default();
	auto device = cuda_::device::current::get();

	auto d_span = cuda_::memory::make_unique_span<int>(device, N);
	auto h_span = cuda_::memory::host::make_unique_span<int>(N);
	...
	auto h_d_span = cuda_::memory::host::make_unique_span<int>(N);
	cuda_::memory::copy(h_d_span, d_span);
