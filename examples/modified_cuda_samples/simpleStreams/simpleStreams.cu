...
int main(int argc, char **argv)
{
	...
	auto d_a = cuda::memory::device::make_unique<int[]>(n);
	auto d_c = cuda::memory::device::make_unique<int>();
	...
	cuda::event_t start_event(use_blocking_sync);
	cuda::event_t stop_event(use_blocking_sync);
