...
int ipcRecvShareableHandle(ipcHandle *handle, shared_allocation_handle_t *shHandle) {
  struct msghdr msg = {0};
  struct iovec iov[1];
...
int ipcRecvShareableHandles(ipcHandle *handle,
                            std::vector<shared_allocation_handle_t> &ShareableHandles) {
  for (std::size_t i = 0; i < ShareableHandles.size(); i++) {
...
int ipcCloseShareableHandle(shared_pool_handle_t shared_pool_handle) {
  return close(shared_pool_handle);
}
...
int ipcCloseShareableHandle(shared_allocation_handle_t shHandle) {
