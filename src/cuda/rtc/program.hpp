	...
	const char *const *header_names)
{
	...
	auto status = nvrtcCreateProgram(&program_handle, program_source, program_name, (int) num_headers, header_sources, header_names);
	throw_if_error<cuda_cpp>(status, "Failed creating " + detail_::identify<cuda_cpp>(program_name));
	...
	string_view program_source)
{
	...
	throw_if_error<source_kind_t::ptx>(status, "Failed creating " + detail_::identify<source_kind_t::ptx>(program_name));
	return program_handle;
}
...
inline void register_global(handle_t<cuda_cpp> program_handle, const char *global_to_register)
{
	...
	throw_if_error<cuda_cpp>(status, "Failed registering global entity " + ::std::string(global_to_register)
...
public: // constructors and destructor
	...
	program_base_t(program_base_t&&) = default;
	~program_base_t() = default;

public: // operators

	program_base_t& operator=(const program_base_t& other) = default;
	program_base_t& operator=(program_base_t&& other) = default;
