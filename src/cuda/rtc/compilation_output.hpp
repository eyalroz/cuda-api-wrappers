...
inline void get_ptx(char* buffer, program::handle_t<cuda_cpp> program_handle, const char *program_name = nullptr)
{
	...
	throw_if_error<cuda_cpp>(status, "Failed obtaining compilation output PTX for compilation of "
		+ identify<cuda_cpp>(program_handle, program_name));
}
...
inline size_t get_nvvm_size(program::handle_t<cuda_cpp> program_handle, const char *program_name = nullptr)
{
	...
	throw_if_error<cuda_cpp>(status, "Failed obtaining output NVVM size for compilation of "
		+ identify<cuda_cpp>(program_handle, program_name));
	return size;
}
...
inline void get_nvvm(char* buffer, program::handle_t<cuda_cpp> program_handle, const char *program_name = nullptr)
{
	...
	throw_if_error<cuda_cpp>(status, "Failed obtaining output NVVM for compilation of "
		+ identify<cuda_cpp>(program_handle, program_name));
}
...
public:
	...
	compilation_output_base_t(const compilation_output_base_t& other)
		:
...
public: // non-mutators
	...
	bool has_ptx() const
	{
		...
		throw_if_error<source_kind>(status, "Failed determining whether compilation resulted in PTX code for "
	...
	bool has_nvvm() const
	{
		...
		if (status == NVRTC_ERROR_INVALID_PROGRAM) { return false; }
		throw_if_error<source_kind>(status, "Failed determining whether the NVRTC program has a compiled NVVM result: "
			+ compilation_output::detail_::identify(*this));
		if (size == 0) {
	...
	link::options_t                         options)
{
	if (not compiled_program.succeeded()) {
		throw std::invalid_argument("Attempt to create a module after compilation failure of "
			+ cuda::rtc::program::detail_::identify<Kind>(compiled_program.program_handle()));
	}
