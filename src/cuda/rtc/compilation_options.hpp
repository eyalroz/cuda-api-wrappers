	...
	bool need_delimited_after_every_option = false)
{
	...
	if (opts.maximum_register_counts.kernel.has_value()) {
		marshalled << opt_start << "--maxrregcount " << opts.maximum_register_counts.kernel;
	}
	if (opts.maximum_register_counts.device_function.has_value()) {
		marshalled << opt_start << "--device-function-maxrregcount " << opts.maximum_register_counts.device_function;
	}
