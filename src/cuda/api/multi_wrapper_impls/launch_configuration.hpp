	...
	span<CUlaunchAttribute> attribute_storage) noexcept(true)
{
	...
	if (config.block_cooperation) {
		auto &attr_value = attribute_storage[attribute_index++];
	...
	if (config.clustering.scheduling_policy != cluster_scheduling_policy_t::default_) {
		auto &attribute = attribute_storage[attribute_index++];
	...
	attribute_storage[attribute_index] = { CU_LAUNCH_ATTRIBUTE_IGNORE, {}};
