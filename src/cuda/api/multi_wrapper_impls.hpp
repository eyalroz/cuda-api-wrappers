...
{
    return detail_::copy(
		stream.handle(), destination_address, destination_context.handle(), source_address,
...
properties_t create_properties(cuda::device_t device)
{
...
{
	virtual_::set_access_mode(address_range_, device, access_mode);
}
...
inline access_mode_t mapping_t::get_access_mode(const device_t& device) const
{
	...
	auto result = cuMemGetAccess(&flags, &location, address_range_.device_ptr() );
	throw_if_error(result, "Failed determining the access mode for "
		+ cuda::device::detail_::identify(device.id())
		+ " to the virtual memory mapping to the range of size "
		+ ::std::to_string(address_range_.size()) + " bytes at " + cuda::detail_::ptr_as_hex(address_range_.data()));
...
#ifndef NDEBUG
	if (accessor == peer) {
		throw std::invalid_argument("A context cannot be used as its own peer");
	}
#endif
