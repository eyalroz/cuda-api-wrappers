...
constexpr attribute_t link_performance_rank = CU_DEVICE_P2P_ATTRIBUTE_PERFORMANCE_RANK;

/// 1 if access is supported, 0 otherwise
constexpr attribute_t	access_support = CU_DEVICE_P2P_ATTRIBUTE_ACCESS_SUPPORTED;

/// 1 if the first device can perform native atomic operations on the second device, 0 otherwise
constexpr attribute_t	native_atomics_support = CU_DEVICE_P2P_ATTRIBUTE_NATIVE_ATOMIC_SUPPORTED;

#if CUDA_VERSION >= 10000
/// 1 if special array interpolatory access operations are supported across the link, 0 otherwise
constexpr attribute_t	array_access_support = CU_DEVICE_P2P_ATTRIBUTE_CUDA_ARRAY_ACCESS_SUPPORTED;
#endif
///@}
