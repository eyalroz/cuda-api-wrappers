...
inline reserved_address_range_t reserve(region_t address_range, alignment_t alignment = alignment::default_)
{
    unsigned long flags { 0 };
    CUdeviceptr ptr;
    auto status = cuMemAddressReserve(&ptr, address_range.size(), alignment,address_range.device_ptr(), flags);
    throw_if_error(status, "Failed making a reservation of " + cuda::memory::detail_::identify(address_range));
    if (ptr != address_range.device_ptr()) {
        throw runtime_error(cuda::status::invalid_value,
            "Reservation result address different from original address"); }
    bool is_owning { true };
    return detail_::wrap(address_range, alignment, is_owning);
}
...
public: // non-mutators
    size_t granularity(granularity_kind_t granuality_kind) const {
        ...
        throw_if_error(status, "Could not determine physical_allocation granularity");
        return result;
    }
...
public: // constructors & destructions
	...
	{
		if (not owning_) { return; }
		auto result = cuMemUnmap(address_range_.device_ptr(), address_range_.size());
